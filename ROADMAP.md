...
- [x] **JSDoc + `@example` coverage across the entire public API** — every exported interface, type alias, and method in `types.ts` now carries a working `@example` snippet; public-facing methods in `KafkaClient` (index.ts) carry `@example` where the JSDoc adds unique implementation detail, and `/** @inheritDoc */` on trivial delegation methods so VS Code and TypeDoc inherit the full documentation (including examples) from `IKafkaClient` without duplication; public items in `errors.ts` (all three error classes), `message/envelope.ts` (`EventEnvelope`, `getEnvelopeContext`, `runWithEnvelopeContext`), and `message/topic.ts` are fully covered; internal classes (`MetricsManager`, `InFlightTracker`, `CircuitBreakerManager`, `AsyncQueue`) and internal functions in `handler.ts`, `pipeline.ts`, `retry-topic.ts`, `dlq-replay.ts` receive descriptive `@param`/`@returns` JSDoc without `@example`; JSDoc convention documented in project memory: public API → `@example`, internal → descriptive only, implementing class → `@inheritDoc`

- [x] **`startTransactionalConsumer(topics, handler, options?)`** — consume-produce with exactly-once semantics; each message is processed inside a dedicated Kafka transaction; the handler receives a `TransactionalHandlerContext` with `tx.send()` and `tx.sendBatch()` that stage outgoing messages inside the transaction; on handler success all staged sends and the source offset commit are applied atomically via `tx.sendOffsets() + tx.commit()` — downstream consumers only see outputs after the commit; on handler failure `tx.abort()` is called automatically, no staged sends become visible, and the source message is redelivered because the offset was never committed; incompatible with `retryTopics: true` (throws at startup); `autoCommit` is always `false` (managed by the transaction); null/empty messages are committed directly without starting a transaction; `TransactionalHandlerContext<T>` added to `types.ts`; 10 unit tests covering: transaction opened per message, full envelope forwarding, sendOffsets + commit on success, tx.send() staging, tx.sendBatch() staging, tx.abort() on handler failure, no commit on failure, null-message skip path, retryTopics guard, ConsumerHandle shape

### 0.7.9

- [x] **`lagThrottle` in `KafkaClientOptions`** — delays `sendMessage`, `sendBatch`, and `sendTombstone` when the observed total lag of a consumer group exceeds `maxLag`; lag is polled via `getConsumerLag()` every `pollIntervalMs` (default `5 000` ms) in a background `setInterval` started by `connectProducer()` and cleared by `disconnect()` / `disconnectProducer()`; while throttled, each send spins in a `100 ms` loop until lag drops or `maxWaitMs` (default `30 000` ms) is exceeded — after which a warning is logged and the send proceeds (best-effort, not hard backpressure); poll errors are silently ignored so a failing admin call never blocks sends; `lagThrottle.groupId` defaults to the client's default group; 7 unit tests covering: no-throttle below threshold, delayed send until lag drops, maxWaitMs fallback, unconfigured client passes through, timer cleanup on disconnect, sendBatch throttling, poll-error transparency

### 0.7.8

- [x] **`startRoutedConsumer(topics, { header, routes, fallback }, options?)`** — dispatches each incoming message to a handler based on the value of a named Kafka header; eliminates `if/switch` boilerplate inside a catch-all handler when one topic carries multiple event types distinguished by a header (e.g. `x-event-type`); messages whose header is absent or unmatched are forwarded to `fallback` if provided, otherwise silently skipped; a thin zero-overhead wrapper over `startConsumer` — all `ConsumerOptions` (retry, DLQ, deduplication, circuit breaker, interceptors) apply uniformly to every route; `RoutingOptions<E>` added to `types.ts`; 9 unit tests covering matched dispatch, full envelope forwarding, fallback on mismatch, fallback on absent header, silent skip without fallback, multiple routes on one topic, ConsumerHandle shape, and ConsumerOptions forwarding

### 0.7.7

- [x] **`startWindowConsumer(topic, handler, { maxMessages, maxMs })`** — accumulates messages into a buffer and flushes the handler when either `maxMessages` messages have arrived or `maxMs` milliseconds have elapsed since the first message in the current window — whichever fires first; semantically distinct from `startBatchConsumer` (which delivers broker-sized batches of unpredictable size) — a window consumer gives explicit control over both batch size and latency, enabling micro-batching for DB writes, event aggregation, or downstream rate-limiting; `WindowMeta { trigger, windowStart, windowEnd }` is passed to the handler on every flush so callers can distinguish size- vs time-triggered flushes; on `handle.stop()` any buffered messages are flushed before the consumer disconnects — no messages lost on clean shutdown; `retryTopics: true` is rejected at startup with a clear error; `WindowConsumerOptions` and `WindowMeta` added to `types.ts`; 13 unit tests covering validation guards, size flush, multi-flush, time flush with fake timers, no-double-flush, WindowMeta fields, shutdown flush, empty-buffer stop, and timer cancellation on stop

### 0.7.6

- [x] **`checkpointOffsets(groupId, checkpointTopic)`** — snapshots the current committed offsets of a consumer group into a Kafka topic; each call appends a new record keyed by `groupId` with `x-checkpoint-timestamp` and `x-checkpoint-group-id` headers, making the checkpoint topic an append-only audit log — use a non-compacted topic to retain history; requires `connectProducer()` to have been called; returns `CheckpointResult { groupId, topics, partitionCount, savedAt }`
- [x] **`restoreFromCheckpoint(groupId, checkpointTopic, options?)`** — reads all checkpoint records for the group from beginning to high-watermark using a short-lived temp consumer, selects the newest checkpoint whose `savedAt ≤ options.timestamp` (or the overall latest when no timestamp is given), and calls `admin.setOffsets` for every topic-partition in that checkpoint; falls back to the oldest available checkpoint (with a warning) when all records are newer than the requested timestamp; throws if no checkpoint exists for the group or if the group is currently running; returns `CheckpointRestoreResult { groupId, offsets, restoredAt, checkpointAge }`; `CheckpointEntry`, `CheckpointResult`, `CheckpointRestoreResult`, `RestoreCheckpointOptions` added to `types.ts`; 16 unit tests covering empty topic, missing checkpoints, latest restore, timestamp-based nearest selection, oldest-fallback, foreign-group filtering, malformed message skip, `checkpointAge` correctness, and default groupId

### 0.7.5

- [x] **`readSnapshot(topic, options?)`** — reads any topic from the beginning to its current high-watermark and returns a `Map<key, EventEnvelope<T>>` with the latest value per key; tombstone records (null-value messages) remove the key from the map, consistent with log-compaction semantics; optional `schema` validates each payload (invalid messages are skipped with a warning, not thrown); optional `onTombstone` callback fires for each removed key; temp consumer is not registered in `this.consumers` and is disconnected immediately after all partitions are read; `ReadSnapshotOptions` added to `types.ts`; 13 unit tests covering empty topics, key overwrite, tombstone removal, multi-partition reads, HWM boundary, schema validation, and disconnect
- [x] **SonarLint cleanup in `kafka.client/index.ts`** — S7773: `parseInt`/`isNaN` → `Number.parseInt`/`Number.isNaN` throughout; S4325: removed unnecessary `as string` type assertions on `message.offset` and `topic`; S3358: nested ternary for key extraction replaced with `if/else`; S3776: `setupConsumer` cognitive complexity reduced from 20 → ≤15 by extracting `ensureConsumerTopics` private method (handles base-topic, DLQ, and dedup topic creation); S3776: `readSnapshot` eachMessage body extracted into `applySnapshotMessage` private method; S7735: `stopConsumer` negated-condition guard flipped to `if (groupId === undefined)` early-return with the "stop all" branch first; S107: `launchRetryChain` parameter count reduced from 8 to 4 by bundling `retry`, `dlq`, `interceptors`, `schemaMap`, `assignmentTimeoutMs` into a single `opts` object
